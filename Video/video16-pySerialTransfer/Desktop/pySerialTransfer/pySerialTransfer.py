...
class SerialTransfer:
    def __init__(self, port, baud=38400, restrict_ports=True, debug=True, byte_format=BYTE_FORMATS['little-endian'], timeout=0.05, write_timeout=None):
        '''
        Description:
        ------------
        ...
        ###################################################################################################
        #####  you must also uncomnent lines 131-142 above  ( and tab the self.port under the else )
        ##################################################################################################
    ...
    def stuff_packet(self, pay_len):
        ...
        Description:
        ------------
        Enforces the COBS (Consistent Overhead Stuffinguleset across
        ...
        print("Position for ref_byte:",ref_byte)
        if (not ref_byte == -1) and (ref_byte <= MAX_PACKET_SIZE):
            for i in range(pay_len - 1, -1, -1):
                if self.tx_buff[i] == START_BYTE:
                    self.tx_buff[i] = ref_byte - i
                    ref_byte = i
        print("Stuffed tx:",self.tx_buff[:pay_len])
    ...
    def send(self, message_len, packet_id=0):
        ...
        try:
            ...
            print("send Size:",len(stack),"msg:",stack)
            ...
            if self.connection.in_waiting:
                while self.connection.in_waiting:
                    ...
                    if self.state == State.FIND_START_BYTE:
                        ...
                        found_checksum = self.crc.calculate(self.rx_buff, self.bytes_to_rec)
                        print("Checksum  rec, found:", rec_char, found_checksum)
                        #print(self.bytes_to_rec,self.rx_buff)
