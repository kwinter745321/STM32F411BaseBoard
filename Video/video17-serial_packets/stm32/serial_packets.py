...
# from https://github.com/zapta/serial_packets_py dated June 21, 2023
# Zapta's code is implemented on Desktop and uses pyserial and pyserial-asyncio
#

# I re-wrote the code was re-written for a MCU but does not use asyncio (although it could)
# as I thought it would be easier to understand.
...
class PacketHDLC(object):
    ...
    def __init__(self, uart, max_len=1024):
        ...
        self._state = PacketHDLC.STATE_START
    ...
    def buildcommand(self, cmd_id, endpoint, data):
        packet = bytearray()
        packet.append(PacketType.COMMAND)
        #packet.extend(int.to_bytes(cmd_id, 'big'))
        cmd = bytearray(4)
    ...
    def buildmessage(self, endpoint: int, data: bytearray):
    ...
    def byte_stuffing(self, packet: bytearray):
        ...
        result = bytearray()
        result.append(PacketHDLC.PACKET_START_FLAG)
        for byte in packet:
            if byte in [PacketHDLC.PACKET_START_FLAG , PacketHDLC.PACKET_END_FLAG, PacketHDLC.PACKET_ESC]:
                result.append(PacketHDLC.PACKET_ESC)
                result.append(byte ^ 0x20)
            else:
                result.append(byte)
        result.append(PacketHDLC.PACKET_END_FLAG)
        return result
...
#    def __init__(self):
    ...
    def receive_byte(self, b: int) :
        ...
        if not self.__in_packet:
            if b == PacketHDLC.PACKET_START_FLAG:
        ...
        if b == PacketHDLC.PACKET_END_FLAG:
            ...
            if self.__pending_escape:
                ...
                try:
                    ...
                    self.dest_ep = 0
                print("--Receive_byte MYFLAG:",self.myflag)
        ...
        if self.__pending_escape:
            ...
            b1 = b ^ 0x20
            if b1 not in [PacketHDLC.PACKET_START_FLAG, PacketHDLC.PACKET_END_FLAG, PacketHDLC.PACKET_ESC]:
                # logger.error(
    ...
    def process_packet(self):
        ...
        hi = foundcrc[0]
        lo = foundcrc[1]
        print("--Found crc high-byte: %02x low byte: %02x" %(hi,lo))
        packet_crc = hi << 8
        packet_crc += lo
        computed_crc = self.crc16_ccitt(0xFFFF, bytes(rx_bfr[:-2]))
        print("--Computed crc:",computed_crc)
        pay_crc = self.crc16_ccitt(0xFFFF, bytes(rx_bfr))
        print("--Payload crc:",pay_crc)
